...
| **Node** | A single element in a Checklist's tree — either a `Category` or an `Expense Item`, fixed by an explicit **node type** (the two kinds are persisted in separate tables — see ADR-0007). Every Node carries a stable integer `id` (the canonical reference, used by the API/CLI — unique **within its table**; `category` and `expense_item` ids are separate spaces, so references also carry a `kind` — see ADR-0010), a `name` (not required unique — sibling names may repeat), a `position` (its meaningful order among siblings, which may interleave categories and items), and a `done` checkbox. | — | `pinned` |
